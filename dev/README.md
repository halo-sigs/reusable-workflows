...
- [ ] For changes under `app-store-release-action`, run `pnpm install --frozen-lockfile`, `pnpm exec biome check src`, `pnpm run typecheck`, and `pnpm run build` in that directory. Commit the generated `dist/index.js`; regenerate `pnpm-lock.yaml` with pnpm when dependencies change.
